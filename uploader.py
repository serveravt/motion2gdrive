...
class MotionUploader:
    ...
    def upload_videos_in_folder(self, folder_path):
        """Upload all videos from folder to the specified folder. Then optionally send an email and optionally delete the local file."""
        onlyfiles = [f for f in listdir(folder_path) if isfile(join(folder_path, f))]
        for s in onlyfiles:
            str(s)
            MotionUploader(cfg_path).upload_video(join(folder_path, s))
    ...
    try:
        ...
        if len(sys.argv) < 3:
            exit('Motion Uploader - uploads videos and snapshots to Google Drive\n'+
                 ...
                 '            allvideos : upload all .avi videos from folder\n'+
        ...
        elif option.lower() == 'allvideos':
            MotionUploader(cfg_path).upload_videos_in_folder(vid_path)
        else:
